...
function initSegmentSection {
  ...
  for (( i=0; i<${#segment_list[@]}; i++ )) ; do
    ...
    $SCRIPT_SEGMENT_HANDLER "${SEGMENT_UPDATE_INTERVAL_DEFAULT}s" "$segment_name" $1 $i "$current_segment_background" "$current_segment_foreground" "$previous_segment_background" "$next_segment_background" $implementation &
